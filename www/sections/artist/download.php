...
$GroupIDs = legacy_array_keys($ArtistRoles);
...
$SQL .= "
	...
	t.Size
FROM torrents AS t
	JOIN torrents_group AS tg ON tg.ID = t.GroupID AND tg.CategoryID = '1' AND tg.ID IN (".legacy_implode(',', $GroupIDs).")
...
while (list($Downloads, $GroupIDs) = $Collector->get_downloads('GroupID')) {
	...
	$TorrentFilesQ = $DB->query("
		...
		FROM torrents_files
		WHERE TorrentID IN (".legacy_implode(',', legacy_array_keys($GroupIDs)).')', false);
