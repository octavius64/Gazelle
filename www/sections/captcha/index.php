...
function get_font() {
	global $CaptchaFonts;
	return SERVER_ROOT.'/classes/fonts/'.$CaptchaFonts[mt_rand(0, legacy_count($CaptchaFonts) - 1)];
}
...
function make_captcha_img() {
	...
	$BGImg = imagecreatefrompng(SERVER_ROOT.'/captcha/'.$CaptchaBGs[mt_rand(0, legacy_count($CaptchaBGs) - 1)]);
