...
if ($Hour != $NextHour || $_GET['runhour'] || isset($argv[2])) {
	 ...
	 foreach ($Criteria as $L) { // $L = Level
		$Query = "
				...
				WHERE PermissionID = ".$L['From']."
					AND Warned = '1970-01-01 00:00:00'
	...
	$DB->query("
		...
		WHERE TimeFilled < (NOW() - INTERVAL 7 DAY)
			AND TimeFilled != '1970-01-01 00:00:00'");
	...
	while (list($UserID) = $DB->next_record()) {
		$Cache->begin_transaction("user_info_$UserID");
		$Cache->update_row(false, array('Warned' => '1970-01-01 00:00:00'));
		$Cache->commit_transaction(2592000);
	}

	$DB->query("
		UPDATE users_info
		SET Warned = '1970-01-01 00:00:00'
		WHERE Warned < '$sqltime'");
	...
	$DB->query("
		...
		WHERE i.RatioWatchEnds != '1970-01-01 00:00:00'
...
if (!$NoDaily && $Day != $NextDay || $_GET['runday']) {
	...
	$UserQuery = $DB->query("
			...
			WHERE m.Uploaded/m.Downloaded >= m.RequiredRatio
				AND i.RatioWatchEnds != '1970-01-01 00:00:00'
				AND m.can_leech = '0'
				AND m.Enabled = '1'");
	...
	foreach ($OffRatioWatch as $UserID) {
		$Cache->begin_transaction("user_info_heavy_$UserID");
		$Cache->update_row(false, array('RatioWatchEnds' => '1970-01-01 00:00:00', 'RatioWatchDownload' => '0', 'CanLeech' => 1));
		$Cache->commit_transaction(0);
	...
	$DB->query("
		...
		WHERE um.PermissionID IN ('".USER."', '".MEMBER	."')
			...
			AND um.LastAccess != '1970-01-01 00:00:00'
			AND ui.Donor = '0'
			AND um.Enabled != '2'
	...
	$DB->query("
		...
		FROM users_info AS ui
			JOIN users_main AS um ON um.ID = ui.UserID
		WHERE um.LastAccess = '1970-01-01 00:00:00'
