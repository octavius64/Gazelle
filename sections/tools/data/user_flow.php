...
$RS = $DB->query("
		...
		FROM (
				...
				FROM users_info
				WHERE JoinDate != '1970-01-01 00:00:00'
				GROUP BY Date
			) AS j
			LEFT JOIN (
				SELECT
					...
					COUNT(UserID) AS Flow
				FROM users_info
				WHERE BanDate != '1970-01-01 00:00:00'
					AND BanReason = '1'
				GROUP BY Date
...
<?	if (!isset($_GET['page'])) { ?>
	<div class="box pad">
		<img src="https://chart.googleapis.com/chart?cht=lc&amp;chs=820x160&amp;chco=000D99,99000D&amp;chg=0,-1,1,1&amp;chxt=y,x&amp;chxs=0,h&amp;chxl=1:|<?=legacy_implode('|', $Labels)?>&amp;chxr=0,0,<?=$Max?>&amp;chd=t:<?=legacy_implode(',', $InFlow)?>|<?=legacy_implode(',', $OutFlow)?>&amp;chls=2,4,0&amp;chdl=New+Registrations|Disabled+Users&amp;chf=bg,s,FFFFFF00" alt="User Flow vs. Time" />
	</div>
<?	} ?>
	<div class="linkbox">
<?
